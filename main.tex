...
The \texttt{astropy.units} subpackage provides ...
...
\begin{description}
    ...
    \item[Interaction with \texttt{numpy} arrays] ...
\end{description}
...
% Adrian, Erik
\inlinecomment{APW}{Do we want to put in a PDF of the transform graph?}
\inlinecomment{EJT}{I think so?  It looks pretty slick...}
...
objects. Some of the machinery also relies on the Essential Routines of Fundamental Astronomy (ERFA) C library for some of the critical underlying transformation machinery.
\inlinecomment{EJT}{Need to create a Zenodo DOI for ERFA and then cite it here.}
...
System (ICRS) reference frame could be represented as spherical (right ascension, declination, and distance from solar system barycenter) or Cartesian coordinates ($x$, $y$, $z$ with the origin at ).
They can therefore change representations independent of being transformed to other reference frames (e.g., the Galactic coordinate frame).
...
\begin{description}
    ...
    \comment{EJT}{APW, you may want to give this item a quick read-over?  If it looks good just delete this comment}
    Also new is support for computing ephemeris of major solar system bodies and outputting the resulting positions as coordinate objects.
    These ephemerides can be computed either using analytic approximations from ERFA (which requires no data download beyond the source code), or from downloaded JPL ephemerides (which requires the \package{jplephem}\footnote{\url{https://github.com/brandon-rhodes/python-jplephem}} optional dependency).
    The \texttt{get\_body} and \texttt{get\_body\_barycentric} functions provide the gateway to this functionality, yielding \texttt{SkyCoord} objects given a specific body and choice of ephemeris source.
...
\begin{description}
	...
	\item [FITS: command-line scripts (fitsheader, fitsinfo)]
		TODO.
	...
	\item [Misc: YAML serialization (v1.3)]
		The new \astropy{astropy.io.misc.yaml} module
		allows converting astropy objects into a standard YAML format.
