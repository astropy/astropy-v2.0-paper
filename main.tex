...
community.
A key element of the project is the \astropy core package \astropypkg, which serves as the
...
Initially separate packages with typically rapid initial development and new experimental features benefit from having a release cycle that is not tied
to the \astropypkg core.
Affiliated pacakges are listed on the main astropy website and advertized to the community through astropy mailing lists, so becoming an affiliated package is sa good way for new and existing packages to gain exposure while at the same time promoting \astropy's high standard for code and documentation quality. This process of listing and promoting affiliated packages is one way in which the \astropy project tries to help with code reuse in the astronomical community. Packages can become affilited to \astropy by applying for this status on a public mailing list. The coordination comittee (sect.~\ref{sect:coordcom}) reviews such requests and issues recommendations for the improvement of a package where possible.
