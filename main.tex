...
\inlinecomment{BMS}{Move this subsection before the APE one, as that one has some repeated content in the first sentences that belongs here}
...
$\approx -0.47$), meaning most of the commits were and are contributed by a
...
We summarize key new features or updates to \package{astropy.table} below:
\begin{description}
    \item[Support for grouped table operations] Some tables contain data that
        naturally form groups.
        For example, a table of photometry for galaxies, stars, and quasars.
        We may then want to split the table or a column into these groups and
    ...
    \item[Support for table concatenation] Tables can be combined in several
        different ways.
    ...
    \item[Mixin columns for time and coordinates] The default data containers
        for \texttt{Table} objects are \texttt{Column} objects.
        However, sometimes the data for a given column is better represented as
        a \texttt{Quantity}, or a sky coordinate (\texttt{SkyCoord}), or an
        astronomical \texttt{Time} object.
        The \texttt{Table} object now supports these and other objects to serve
        as array-like containers for column data through the ``mixin'' protocol.
    \item[Transform to and from \texttt{Pandas} \texttt{DataFrame} objects]
...
\begin{description}
    ...
    \item[\texttt{.ascii}: fast readers and writers]
        ...
        available for the following formats: \texttt{basic},
        \texttt{commented\_header}, \texttt{csv}, \texttt{no\_header},
        \texttt{rdb}, and \texttt{tab}.  On average the new engine is about
...
\package{astropy.modeling} provides an efficient way to instantiate the same type of model with many different sets of parameter values by passing the \texttt{n\_models} argument on instantiation, which sets the number of models to instantiate. The the values for each parameter in the model set must be passed as iterables of size \texttt{n_models}. This creates a model set that can be efficiently evaluated. Model sets are most useful for efficiently fitting multiple linear models of the same kind.
...
\astropypk models with external fitters. An example of this is
...
%
\par Development has already been continued after the release of v2.0 of
\astropypkg. The most significant development for the next release will
be removing python 2 support \citep{ape10}, the v3.0 \astropypkg release
will only support python 3.  This will allow the use of the new
functionality in python 3, simplify the code base, and reducing the
testing overhead.   The v3.0 release is currently scheduled for January
2018.
%
\par In the next major release after v3.0, scheduled for the summer of
2018, the focus will be on optimization of the code and improved
documentation. To prepare for this release, software for testing and
monitoring the performance of the code are being developed.   Less
functionality may be introduced in this release while the focus will be
primarily on improved performance.  In addition, a new suite of tutorials
and documentation are being prepared for this release.
%
\par Beyond the core code, the \astropy project is also further
developing the affiliated packages.  This includes elevating a suite of
packages into part of the core project. While these may not be integrated
into the \astropypkg core code, these projects provide code that is
useful to the astronomical community and meet the testing and
documentation standards of \astropy.   Some of these new efforts includes
an initiative to develop tools for spectroscopy
\citep[\package{specutils}, \package{specreduc}, \package{specviz}]
