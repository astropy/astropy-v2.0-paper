...
repository \citep[][]{adrian_price_whelan_2018_1211397}\footnote{Codebase: \url{https://github.com/astropy/astropy-v2.0-paper}}.
...
the basis for future work.  In cases where consensus cannot be reached, the
...
development as compared to the core package.
...
In addition to representing celestial
    ...
    \astropy-affiliated package \package{astroplan}\citep{astroplan}.
...
function to produce the RGB image from three SDSS filter images ($g$, $r$, $i$).  The image  on the left shows
the image with the default parameters, whereas the image on the right has paramets set to show a greater dynamical range.
\label{fig:ngc6977}}
\end{figure}
...
ecosystem, the actual usage of this package is extremely widespread\footnote{Approximately 30\%
of the ci-helpers contributor are coming from outside the astropy community and
there are currently 292 repositories on github that has ci-helpers in their travis configuration file.}. The current
...
the \astropypkg core package (version 3.0) began.
...
\astropypkg version 3.0 was released in February 2018.

In the next major release after version 3.0, the focus will be on algorithm
...
Foundation of South Africa. MVB was supported by NASA's Planetary Astronomy Program.
TLA was supported by NASA contract NAS8-03060. Support for EJT. was provided by NASA
...
Gemini partnership of Argentina, Brazil, Canada, Chile, and the United States of America.  YPB was supported by
