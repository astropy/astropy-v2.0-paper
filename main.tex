...
l\section*{\textit{Notes and guidelines (to be removed)}}
...
%draft by hamogu
\inlinecomment{TODO}{subpackage vs sub-package need to be standardized throughout the paper}

\inlinecomment{Tom R}{at the moment this section doesn't take into account that APEs can be unrelated to code (e.g. APE 8 for the code of conduct)}
...
be \citep{ape1}. These APEs are discussed and refined before much work is invested
...
As of version 2.0, \astropypkg contains $212244$ lines of code\footnote{This line count includes comments, as these are often as important for maintainability as the code itself.  Without comments there are $142197$ lines of code} contributed by $232$ unique
contributors over $19270$ \texttt{git} commits.
The commit distribution follows an approximate power-law (with log-slope of $\sim -0.47$), meaning most of the commits were and are contributed by a relatively small fraction of the contributor pool. $\sim 50$ percent of the commits were contributed by the top $4$ contributors, while $\sim 90$ percent of the commits came from the top $24$.
...
contain only bugfixes but no new features \citep{ape2}.
% The timing of
%
...
The \package{Units} and \package{Quantities} packages were described in detail in \cite{astropy}.   
...
Astronomers using \package{Constants} should take particular note of the
constants provided for Earth, Jupiter and the Sun. Following IAU 2015 
Resolution B3 \citep{iau2015b3}, nominal
values are now given for mass parameters and radii. The nominal values 
will not change even as ``current best estimates'' are updated.

\inlinecomment{DavidS: we'd like to build on this to allow use of a
context manager for selecting a version of constants. Here, or in the Future
section?}
...
% timj: The preferred SOFA reference is usually https://ui.adsabs.harvard.edu/#abs/2011SchpJ...611404H/abstract rather than a URL to the SOFA web site.
