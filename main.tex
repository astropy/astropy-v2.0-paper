...
the \astropypkg~core but is currently undergo rapid development and can 
...
% Tom R: unit support description goes here and the sentence below is to be folded in with it.

Unit support in \texttt{modeling} has enabled implementation of the blackbody \citep{Rybicki1979} model, which requires units to make sense. It is available in \package{astropy} 2.0 as \texttt{BlackBody1D}.
...
One of the most common tasks by astronomers dealing with observational data is to make figures with images that include the correct coordinates (formatted e.g. as sexagesimal values) and optionally with a graticule showing the lines of longitude/latitude.


The \package{visualization} package
% Tom R. will work on this on Oct 18 and/or 19.
...
\citep{Lupton2004} describe an “optimal” algorithm for producing red-green- blue composite images from three separate high-dynamic range arrays. The \package{visualization} package provies a convenience function to create such an image.  It also includes an associated set of classes to provide alternate scalings.  The SDSS SkyServer color images were made using a variation on this technique.
% mention LSST contribution (Zenodo ref?)
% include images here
...
The \package{astropy.stats} package provides
statistical tools that are useful for or specific to
astronomy and are not found in or extend the available
functionality of other \python\
...
\inlinecomment{BMS}{Can we highlight in a sentence for each of the ones below how they are different from e.g. the scipy equivalents?}
\inlinecomment{LDB}{Thanks.  I did this for sigma clipping.  Scipy doesn't have functions for the others...}
...
{\bf Sigma Clipping:}
Sigma clipping provides a fast method to identify outliers in a distribution. For a distribution of points, a center and a standard deviation are calculated. Values which are a set number of sigma times the standard deviation away from the center are rejected. The process can be iterated to further reject outliers.  It should be noted that the \package{scipy} sigma clipping function provides a subset of functionality of the \package{astropy.stats} function.  The \package{astropy.stats} version can take a masked array as input and it allows the user to specify the number of iterations, the data axis on which to perform the clipping, and the functions used to calculate the center and standard deviation in the data.

{\bf Median Absolute Deviation:}
The median absolute deviation (MAD) is a measure of the spread of a distribution and is defined as median(abs(a - median(a))). For a normal distribution, the MAD is related to the standard deviation by a factor of 1.4826 (the normal inverse cumulative distribution function evaluated at probabilty $P=3/4$), and a convenience function is available to apply the conversion.

{\bf Biweight Estimators: }
A set of functions are included in the \package{stats} package that use the biweight formalism. These functions have long been used in astronomy, particularly to calculate the velocity dispersion of galaxy clusters \citep{Beers1990}.

{\bf Circular Statistics: }
A set of circular statistical estimators based on [XXX] are implemented in the \package{stats} package.  These functions provide measurements of the circular mean, variance, and moment.   For all of these functions, they work with both numpy.ndarrays (assumed to be in radians) and Quantity objects.  In addition, the package includes tests for Raleigh Test [XXX], vtest [XXX], and a function to compute the maximum likelihood estimator for the parameters of the von Mises distribution [XXX].
...
\software{\package{astropy} (\citealt{astropy}),
          ...
          \package{Cython}(\url{http://cython.org}),
          }
