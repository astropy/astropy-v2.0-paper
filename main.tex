...
(\texttt{ra}) and declination ((\texttt{dec})), whereas in the Galactic frame,
the spherical angles are Galactic longitude (\texttt{l}) and latitude
(\texttt{b}).
...
underlying \texttt{Representation} object that interally stores the coordinate
data.
...
todo: The typical user doesn't have to interact with the frame classes or
representation classes directly.
Instead, use the \texttt{SkyCoord} object...
