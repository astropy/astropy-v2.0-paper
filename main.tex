...
The \package{astropy.nddata} subpackage provides three types of functionality: concrete classes for representing multi-dimensional gridded data like CCD images and utilities for manipulating multi-dimensional images and high-level classes that define an interface for representing generic gridded data that is intended primarily for developers of other packages.

The \texttt{CCDData} class provides I/O to FITS files and uses data structures from \astropypkg, like WCS, to represent the contents of a file abstractly. This class supports basic arithmetic operations on image, including error propagation in limited cases and slicing of images that also appropriately slices the image mask and error arrays, if present.

The \package{astropy.nddata.utils} module provides utilities that can operate on either plain NumPy arrays or any of the classes in the \package{astropy.nddata} subpackage.  It features a class for representing two-dimensional image cutouts, allowing one to easily link pixels in the cutout to pixels in the original image or from the image to the cutout, and to overlay the cutout on images. Functions to enlarge or reduce an image by doing block replication or reduction are also provided.

At a higher level, classes are provided which define a minimal interface for gridded data with attributes for accessing metadata, WCS and other common traits. These serve as a useful base for package authors wishing to develop their own classes for specific use cases and as containers for exchanging gridded data.
...
Project also maintains and distributes several infrastructure packages that are
...
\par \astropy provides a \package{package-template}\footnote{\url{https://github.com/astropy/package-template/}}
package that any \python package is welcome to
