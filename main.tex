...
simply means using these packages as ``building blocks'' (E.g., the ubiquitous
...
However, the boundaries of when this re-use is called for often ambiguous.  Some
...
\package{astropy.units}, or \package{astropy.table} when it began). While some
of this functionality is ``given back'' to the wider community (E.g., myriad bug
fixes for the enabling technologies listed above, and some of the testing and
documentation architecture), in general, careful consideration is taken of
whether the additional effort to make a piece of \astropy, and it is only
``extracted'' when it seems plausible that the ongoing effort is worth the
reward in the form of additional contributors or maintainers.
...
\package{pandas} data frames \citep{pandas} but with modifications for
...
objects and \package{pandas} data frames using the \texttt{Table.to_pandas()}
and \texttt{Table.from_pandas()} methods. \texttt{Table.to_pandas()} is,
...
enable these areas are significant, so no one has yet volunteered to do this
major undertaking.
